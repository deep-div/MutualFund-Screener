...
interface FundTableProps {
  ...
  onWatchlistSchemeAdded?: (scheme: Pick<SchemeSearchItem, "external_id" | "scheme_sub_name" | "scheme_sub_category">) => void;
}
...
const FundTable = ({
  ...
  const addSchemeToWatchlist = (scheme: Pick<SchemeSearchItem, "external_id" | "scheme_sub_name" | "scheme_sub_category">) => {
  ...
  useEffect(() => {
    fetchPage(0, false);
  }, [filterKey, sortKey, sortDir, isWatchlist, watchlistExternalIdsKey]);
  ...
  const shouldLockTableScroll = showEmptyWatchlistState;
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden min-h-0">
      ...
      <div className="border-b border-border px-3 py-3 sm:px-4 sm:py-4 lg:px-6">
        ...
        >
          <div ref={collapsibleRef} className="min-h-0 overflow-hidden">
        ...
        <div
          ...
          onScroll={(event) => {
            ...
            setIsHeaderCollapsed((previous) => {
              if (!previous && scrollTop > 30) {
                const collapsibleHeight = collapsibleRef.current?.offsetHeight ?? 0;
                // Only collapse if scrollTop won't be pushed below the expand threshold
                if (maxScrollTop - collapsibleHeight < 10) return previous;
                return true;
              }
              if (previous && scrollTop < 10) return false;
              return previous;
            });
          }}
        >
          ...
          {!loading && items.length === 0 && !error && !showEmptyWatchlistState && (
