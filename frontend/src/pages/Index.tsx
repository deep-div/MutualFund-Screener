...
import { listSchemes, SchemeSearchItem } from "@/services/mutualFundService";
import {
...
const Index = () => {
  ...
  const handleWatchlistSchemeAdded = useCallback(
    (scheme: Pick<SchemeSearchItem, "external_id" | "scheme_sub_name" | "scheme_sub_category">) => {
      const subCategory = String(scheme.scheme_sub_category ?? "").trim();
      if (!subCategory) return;
      setFilterValues((prev) => {
        const existingSubCategories = Array.isArray(prev["scheme_sub_category"]?.value)
          ? (prev["scheme_sub_category"].value as string[])
          : prev["scheme_sub_category"]?.value
          ? [prev["scheme_sub_category"].value as string]
          : [];
        if (existingSubCategories.includes(subCategory)) return prev;
        const newSubCategories = [...existingSubCategories, subCategory];
        const group = SCHEME_SUB_CATEGORY_GROUPS.find((g) => g.options.includes(subCategory));
        const schemeClassValue = group?.schemeClassValue ?? group?.label;
        const existingSchemeClasses = Array.isArray(prev["scheme_class"]?.value)
          ? (prev["scheme_class"].value as string[])
          : prev["scheme_class"]?.value
          ? [prev["scheme_class"].value as string]
          : [];
        const next: FilterValueMap = {
          ...prev,
          scheme_sub_category: { value: newSubCategories },
        };
        if (schemeClassValue && !existingSchemeClasses.includes(schemeClassValue)) {
          next["scheme_class"] = { value: [...existingSchemeClasses, schemeClassValue] };
        }
        return next;
      });
    },
      ...
      <div className="relative flex flex-1 overflow-hidden page-dimmable">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <FundTable
            ...
            onWatchlistSchemeAdded={handleWatchlistSchemeAdded}
            onMetaChange={(meta) =>
