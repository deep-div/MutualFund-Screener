...
def run_workflow():
    ...
    update_workflow_run(run_id, workflow_status="running", started_at=datetime.now(timezone.utc))
    ...
    try:
        ...
        try:
            ...
            update_workflow_run(run_id, workflow_status="failed", ingestion_status="failed", ingestion_error=str(e), completed_at=datetime.now(timezone.utc))
            raise
        ...
        try:
            ...
            update_workflow_run(run_id, workflow_status="failed", metrics_status="failed", metrics_error=str(e), completed_at=datetime.now(timezone.utc))
            raise
        ...
        try:
            ...
            update_workflow_run(run_id, workflow_status="failed", db_status="failed", db_error=str(e), completed_at=datetime.now(timezone.utc))
            raise

        update_workflow_run(run_id, workflow_status="success", completed_at=datetime.now(timezone.utc))
